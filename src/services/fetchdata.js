...
setLoadingAPI(true); 
 ...
 .then((res) => res.json()) 
 .then(async (responseData) => { const formatted = responseData.data.map((item) => ({ name: item.VID_FILENAME, id: item.VID_ID, duration: "-", status: "Okay", size: item.VID_SIZE_GB, type: item.VID_FILENAME.split("_")[0] || "Other" })); 
 ...
 const batchFetchMetadata = async (videos, batchSize = 20) => { 
    ...
    const withDurations = await batchFetchMetadata(formatted, 20); 
    setRazunaData(withDurations); 
    setFilteredDataRazuna(withDurations); 
    setLoadingAPI(false); }) 
    .catch((err) => { console.error("Fetch Error:", err); setLoadingAPI(false); }); }, [selectedSource]);